...
The model also has only two drivers, photosynthetically active radiation (PAR) and air temperature. We pass these into the model as a data frame "inputs" with columns "PAR" and temp" (see **Drivers**). The model parameters are similarly passed in through the data frame "params" and are defined in the **Parameters** section below. Finally the model parameter "timestep" controls the model timestep and defaults to 30 min (1800 seconds) as this is the timestep of the meteorological inputs and eddy flux data that we'll use for calibration.
...
As defined, our SSEM model only predicts one timestep into the future. To run this model forward for a full year (or longer) we're going to define a helper function `ensemble_forecast` that runs SSEM recursively, taking the outputs from one timestep and feeding them back in as the initial conditions for the next timestep.
...
likelihood <- function(param,Obs){
  ...
  print(param)
  Model = ensemble_forecast(X = Xbar,
                            params = param[,1:9],
                            inputs = inputs[,2:3]) 
...
```{r}
calibrated.ensemble = ensemble_forecast(X = X.orig,
                                    params = params_new,
                                    inputs = inputs[,2:3], ## don't feed date col'n to model 
                                    verbose= TRUE) 
```
